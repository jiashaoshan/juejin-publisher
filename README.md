...
| 模块 | 脚本 | 功能 |
|------|------|------|
| 📝 **AI 文章生成+发布** | `juejin-article-publisher.py` | LLM 读产品网站 → 生成技术文章 → 自动发布到掘金 |
| 💬 **评论区获客** | `juejin_acquisition.py` | 搜索文章 → AI 四维评分 → LLM 生成评论 → 自动评论+私信 |
| 📄 **文章查询** | `query_article.py` | 查询/管理已发布文章 |
...
└───────────┬─────────────┘
            ▼
┌─────────────────────────┐
│  步骤4: 发布到掘金       │  → create_draft → publish
...
| 风格 | 占比 | 示例 |
|------|------|------|
| 赞同补充 | 30% | "说实话，这篇文章挺到位的。我之前也..." |
| 提问讨论 | 25% | "有个疑问想请教下，关于XX这块..." |
| 实战分享 | 25% | "我们团队最近也在搞这块，踩了几个坑..." |
| 案例分析 | 20% | "看了这个案例，让我想起之前做的一个项目..." |
...
| `JUEJIN_DEFAULT_TAG_IDS` | 默认标签 ID (逗号分隔) | `6809640445233070096` (AI) |
